...
function check_env(){
	$items = array(
		...
		'php'     => array('PHP版本', '5.3', '5.3+', PHP_VERSION, true),
...
function check_dirfile(){
	$items = array(
		array('dir', '可写', true, '.'.__GROUP__.'/Uploads'),
		array('dir', '可写', true, RUNTIME_PATH),
		array('dir', '可写', true, APP_PATH.'Admin/Conf'),
		array('dir', '可写', true, APP_PATH.'Common/Conf'),
	);

	foreach ($items as &$val) {
		$item =	realpath('./') . DIRECTORY_SEPARATOR . $val[3];
		if('dir' == $val[0]){
			if(!is_writable($item)) {
				if(is_dir($items)) {
					$val[1] = '可读';
